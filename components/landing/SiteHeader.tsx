...
export function SiteHeaderAlt() {
  ...
  //   Layer 1: header-bg-alt.webp (autumn forest, 1440×859 — Steph's
  //            colour-graded export, re-encoded from a 2.9MB PNG to a
  //            ~405KB WebP so the hero paints immediately instead of
  //            leaving a blank cream backdrop for ~2s on first load)
  //   Layer 2: HTML controls (logo, nav pill, CTA)
...
function NavPill({ className = "" }: { className?: string }) {
  return (
    <nav
      aria-label="Primary"
      className={`flex min-w-0 shrink items-center rounded-full bg-black/30 px-3 py-2 backdrop-blur-sm sm:px-6 sm:py-3 ${className}`}
    >
      <ul className="flex items-center gap-3 overflow-x-auto whitespace-nowrap text-[12px] tracking-tight text-white [scrollbar-width:none] sm:gap-5 sm:overflow-visible sm:text-[16px] md:gap-6 md:text-[18px] [&::-webkit-scrollbar]:hidden">
